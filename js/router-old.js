...
class Router {
  ...
  shouldIntercept(link) {
    ...
    // Don't intercept on object detail pages, properties list page, or pagination buttons
    if (window.location.pathname.includes('/object/') || 
        window.location.pathname.includes('наши обьекты.html') ||
        link.id === 'prevBtn' || link.id === 'nextBtn' || link.id === 'applyFiltersBtn') return false;
    ...
    const href = link.getAttribute('href');
    // Don't intercept on object detail pages or pagination buttons
    if (window.location.pathname.includes('/object/') || link.id === 'prevBtn' || link.id === 'nextBtn' || link.id === 'applyFiltersBtn') return false;
    // Intercept internal links that are not external or anchors
    return href &&
           !href.startsWith('http') &&
           !href.startsWith('mailto:') &&
    ...
    const href = link.getAttribute('href');
    // Don't intercept on object detail pages
    if (window.location.pathname.includes('/object/')) return false;
    // Intercept internal links that are not external or anchors
    return href &&
           !href.startsWith('http') &&
           !href.startsWith('mailto:') &&
